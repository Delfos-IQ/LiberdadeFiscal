...
- [x] Fase 4 — Rendimentos (ex-Taxímetro: Modo Rápido + Avançado, cadeia bruto→líquido, quociente familiar, dependentes, diferencial regional Madeira ✅/Açores ✅ (ambos 30% uniforme sobre a taxa marginal, verificado 18/08/2026), conteúdo educativo sobre o custo total para o empregador; roadmap P3-15 — modo agregado familiar: campo opcional de segundo rendimento quando "conjunta" está selecionado, soma os dois rendimentos coletáveis antes de aplicar o quociente familiar em vez de o aplicar a um só, com a Segurança Social sempre calculada por pessoa; Taxa Adicional de Solidariedade — Art. 68.º-A CIRS — cablada em 18/08/2026, ✅ verificada em 19/08/2026 contra folheto oficial da AT, afeta só rendimentos coletáveis acima de 80.000€/ano)
...
Cobertura atual: 321 testes (motor fiscal, lógica do quiz, catálogo de bens/serviços, parser de QR de faturas [dormant], persistência IndexedDB incluindo o acumulador de "Período" — e o seu reset controlado via `reiniciarPeriodoAtual` — e exportação/importação de dados, integridade dos dados do Glossário fiscal, acessibilidade via axe-core, integração de UI de todos os módulos via jsdom + fake-indexeddb).

...
Cobertura atual: 319 testes (motor fiscal, lógica do quiz, catálogo de bens/serviços, parser de QR de faturas [dormant], cliente do fallback foto+IA [dormant, roadmap P3-17], persistência IndexedDB incluindo o acumulador de "Período" e exportação/importação de dados, acessibilidade via axe-core, integração de UI de todos os módulos via jsdom + fake-indexeddb).

...
- [x] Fase 4 — Rendimentos (ex-Taxímetro: Modo Rápido + Avançado, cadeia bruto→líquido, quociente familiar, dependentes, diferencial regional Madeira ✅/Açores ✅ (ambos 30% uniforme sobre a taxa marginal, verificado 18/08/2026), conteúdo educativo sobre o custo total para o empregador; roadmap P3-15 — modo agregado familiar: campo opcional de segundo rendimento quando "conjunta" está selecionado, soma os dois rendimentos coletáveis antes de aplicar o quociente familiar em vez de o aplicar a um só, com a Segurança Social sempre calculada por pessoa; Taxa Adicional de Solidariedade — Art. 68.º-A CIRS — cablada em 18/08/2026, ✅ verificada em 19/08/2026 contra folheto oficial da AT, afeta só rendimentos coletáveis acima de 80.000€/ano; Independente/recibos verdes com seletor de tipo de atividade — Art. 31.º CIRS, ✅ verificado em 19/08/2026 diretamente na DR, 5 de 8 coeficientes modelados)
...
- [x] Fase 8 — Benchmark OCDE (tax wedge, Taxing Wages 2026, PT/ES/FR/DE/IE/NL/CH) + cartão para partilhar (Web Share API com fallback de download/clipboard — inclui partilha nativa para WhatsApp quando `navigator.share({files})` está disponível); Degradação Monetária (22/08/2026, a pedido do autor) — rota secundária que compara a evolução nominal do 1.º escalão de IRS (2021-2026, Art. 68.º CIRS) com a inflação acumulada (IPC, INE), ambas ✅ verificadas em fonte primária/INE, ver `TAX-METHODOLOGY.md` §6c; Para onde vão os impostos (22/08/2026, a pedido do autor) — rota secundária que mostra grandes rúbricas do gasto público em 2025 (juros da dívida, pensões, saúde, educação) em valores absolutos, sem percentagem do total por não se ter conseguido confirmar com rigor o denominador consolidado; Defesa Nacional ficou deliberadamente de fora por discrepância de +850M€ entre fontes, ver `TAX-METHODOLOGY.md` §6d
...
Cobertura atual: 363 testes (motor fiscal, lógica do quiz, catálogo de bens/serviços, parser de QR de faturas [dormant], persistência IndexedDB incluindo o acumulador de "Período" — e o seu reset controlado via `reiniciarPeriodoAtual` — e exportação/importação de dados, integridade dos dados do Glossário fiscal, acessibilidade via axe-core, integração de UI de todos os módulos via jsdom + fake-indexeddb).

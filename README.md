...
- [x] Fase 6 — Taxas (ex-Impostos anuais/patrimoniais, formulário simplificado a tipo + valor: IMI/IUC/ISV/IMT/Imposto de Selo; motor de cálculo `calcularISV()`/`calcularIUC()`/`calcularImpostoSelo()` já implementado em `data/tax-engine.js` com dados de `data/tax-rules/2026/patrimoniais.js`; IMI tem agora tabela por concelho — 299 dos 308 municípios, 🟡 ESTIMATE, embutida 18/08/2026)
...
Cobertura atual: 312 testes (motor fiscal, lógica do quiz, catálogo de bens/serviços, parser de QR de faturas [dormant], cliente do fallback foto+IA [dormant, roadmap P3-17], persistência IndexedDB incluindo o acumulador de "Período" e exportação/importação de dados, acessibilidade via axe-core, integração de UI de todos os módulos via jsdom + fake-indexeddb).
